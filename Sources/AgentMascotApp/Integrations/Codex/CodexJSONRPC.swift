...
struct CodexAppServerMapper {
    static func event(from frame: CodexJSONRPC) -> AgentEvent? {
        ...
        let role: AgentSessionRole = (p["parentThreadId"]?.string ?? p["thread"]?.object?["parentThreadId"]?.string) == nil ? .main : .subagent
        return AgentEvent(version:1,id:UUID(),agent:.codex,sessionID:sid,role:role,kind:kind,cwd:p["cwd"]?.string,pid:nil,title:"Codex",question:nil,occurredAt:.now)
    }
}

...
struct ClaudeEventMapper {
    static func map(_ data: Data) throws -> AgentEvent {
        ...
        return AgentEvent(version: 1, id: UUID(), agent: .claudeCode, sessionID: sid, role: root["agent_id"] == nil ? .main : .subagent, kind: kind, cwd: root["cwd"] as? String, pid: (root["pid"] as? NSNumber)?.int32Value, title: root["title"] as? String, question: nil, occurredAt: .now)
    }
}

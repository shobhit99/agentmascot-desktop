...
struct StaticMascotView:View {
    ...
    var body:some View { ZStack { if let url=AgentMascotResources.url(forResource:assets.assetName(for:state),withExtension:"svg"),let image=NSImage(contentsOf:url){Image(nsImage:image).resizable().scaledToFit().frame(width:112,height:112)}else{Color.clear.frame(width:112,height:112)};if state == .error{Image(systemName:"exclamationmark.circle.fill").foregroundStyle(.red).offset(x:45,y:-45)}}.accessibilityLabel("Agent Mascot \(state.rawValue)") }
}

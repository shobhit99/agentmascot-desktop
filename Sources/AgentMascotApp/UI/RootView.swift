...
struct RootView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("Agent Mascot").font(.title.bold())
            ...
            } else if model.mainSessions.isEmpty {
                ContentUnavailableView("No agent sessions", systemImage: "terminal")
            } else {
                List(model.mainSessions) { session in
            ...
            Button("Quit Agent Mascot") { NSApplication.shared.terminate(nil) }
                .buttonStyle(.plain)

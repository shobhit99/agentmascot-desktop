...
actor AgentSessionStore {
    ...
    func apply(_ event: AgentEvent) {
        ...
        var session = sessions[k] ?? AgentSession(id: k, agent: event.agent, state: .idle, cwd: event.cwd ?? "", title: event.title ?? event.sessionID, pid: event.pid, questions: [], updatedAt: event.occurredAt)

...
struct InstallationTokenStore {
    ...
    func loadOrCreate()throws->String {
        ...
        let token=Data(bytes).base64EncodedString();let fd=open(url.path,O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW,0o600);guard fd>=0 else{if errno==EEXIST{return try loadValidated()};throw POSIXError(POSIXErrorCode(rawValue:errno) ?? .EIO)};defer{close(fd)}
        let data=Data(token.utf8);let written=data.withUnsafeBytes{Darwin.write(fd,$0.baseAddress,data.count)};guard written==data.count,fsync(fd)==0 else{throw POSIXError(.EIO)};return token
    }
    private func loadValidated()throws->String {

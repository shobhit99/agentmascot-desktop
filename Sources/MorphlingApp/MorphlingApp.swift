...
@main struct MorphlingApplication: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var appDelegate
    var body: some Scene {
        MenuBarExtra("Morphling", systemImage: appDelegate.model.aggregateState == .needsInput ? "questionmark.circle.fill" : "sparkles") {
...
@MainActor final class AppCoordinator {
    ...
    func start(demoState: DemoState = .none) async {
        ...
        do {
            let support=FileManager.default.urls(for:.applicationSupportDirectory,in:.userDomainMask)[0].appendingPathComponent("Morphling")
            let token=try InstallationTokenStore(url:support.appendingPathComponent("bridge.token")).loadOrCreate()
    ...
    private func startDemo(_ state: DemoState) async {
        ...
        let q=AgentQuestion(id:"demo:q",prompt:"Which direction should Morphling take?",choices:[AgentChoice(id:"demo:a",label:"Continue",description:"Proceed with the current plan",value:"continue"),AgentChoice(id:"demo:b",label:"Pause",description:"Wait for more input",value:"pause")],allowsFreeText:true,isMultiSelect:false)
    ...
    private func configureInstallers(support:URL,token:String) {
        ...
        let claude=ClaudeHookInstaller(settingsURL:home.appendingPathComponent(".claude/settings.json"),scriptURL:support.appendingPathComponent("Hooks/morphling-claude-event.sh"))
        ...
        let codex=CodexHookInstaller(configURL:home.appendingPathComponent(".codex/config.toml"),scriptURL:support.appendingPathComponent("Hooks/morphling-codex-event.sh"))
